"""
BPDM minimal-m closure — LEAN version.
Fewer k-points (nk=25), shells 2..6, flush output.
"""
...
def build_B(kx,ky,th,dim,nG):
    b1=2*np.pi*np.array([1,-1/np.sqrt(3)]);b2=2*np.pi*np.array([0,2/np.sqrt(3)])
    K=(2*b1+b2)/3;K1=rot(-th/2)@K;K2=rot(th/2)@K
    sx=np.zeros((3,3),dtype=complex);sx[0,1]=1;sx[1,0]=1
    sy=np.zeros((3,3),dtype=complex);sy[0,1]=-1j;sy[1,0]=1j
    ...
    for l in range(2):
        ...
        ds=dh[0]*sx+dh[1]*sy
        for ig in range(nG):
            idx=no*ig if l==0 else no*nG+no*ig;B[idx:idx+no,idx:idx+no]=ds
    return B

def run(th_deg,ns,m,nk=25):
    ...
    nw=max(m+4,int(dim*0.4));mid=dim//2;lo=max(0,mid-nw//2);hi=min(dim,mid+nw//2)
    widx=list(range(lo,hi))
    all_eps,purs,pvecs=[],[],[]
    for k in kpts:
        ev,vec,_,_,_,_,_=build(k[0],k[1],th,ns)
        B=build_B(k[0],k[1],th,dim,nG)
        sc=[(abs(np.real(vec[:,n].conj()@B@vec[:,n])),n) for n in widx]
        sc.sort(reverse=True);sel=sc[:m];si=[s[1] for s in sel]
        purs.append(np.mean([s[0] for s in sel]))
        all_eps.append(sorted([ev[n] for n in si]))
        pvecs.append(vec[:,si])
    ae=np.array(all_eps);W=np.max(ae)-np.min(ae)
    return {'W':float(W),'mp':float(np.mean(purs)),'minp':float(np.min(purs)),'pv':pvecs,'dim':dim}
...
def main():
    t0=time.time();th=9.6
    ms=[2,4,6,8,10];shells=range(2,7)
    print(f"BPDM MINIMAL-m: Kagome θ={th}°, m={ms}, shells={list(shells)}",flush=True)
    print("="*80,flush=True)
    res={}
    for m in ms:
        print(f"\n--- m={m} ---",flush=True)
        print(f"{'ns':>3} {'dim':>5} {'W/t':>9} {'pur':>5} {'minp':>5} {'Sm':>6} {'Sn':>6} {'Δ%':>6}",flush=True)
        pp=None;pW=None;mr=[]
        for ns in shells:
            t1=time.time();r=run(th,ns,m);dt=time.time()-t1
            if pp is not None:
                S=overlap(pp,r['pv'],m);Sm=float(np.mean(S));Sn=float(np.min(S))
            ...
            mr.append({'ns':ns,'dim':r['dim'],'W':r['W'],'mp':r['mp'],'minp':r['minp'],
                       'Sm':Sm,'Sn':Sn,'dp':dp,'t':dt})
            sm=f"{Sm:.3f}" if not np.isnan(Sm) else "—"
            sn=f"{Sn:.3f}" if not np.isnan(Sn) else "—"
            d=f"{dp:.1f}" if not np.isnan(dp) else "—"
            print(f"{ns:>3} {r['dim']:>5} {r['W']:>9.4f} {r['mp']:>5.3f} {r['minp']:>5.3f} {sm:>6} {sn:>6} {d:>6}  ({dt:.0f}s)",flush=True)
            pp=r['pv'];pW=r['W']
            if dt>90: print(f"  (stopping m={m})",flush=True); break
        res[m]=mr
    
    print(f"\n{'='*80}",flush=True)
    print(f"{'m':>3} {'W_range':>16} {'ratio':>6} {'S_min':>7} {'last2Δ%':>12} {'verdict':>10}",flush=True)
    for m in ms:
        ...
        print(f"{m:>3} {min(ws):.3f}—{max(ws):.3f} {ratio:>6.2f}x {ls:>7} {str([f'{c:.0f}%' for c in l2]):>12} {conv:>10}",flush=True)
